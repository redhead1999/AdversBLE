...
import com.aold.advers.data.ConnectionState
import com.aold.advers.data.TempHumidityResult
import com.aold.advers.data.TemperatureAndHumidityReceiveManager
...
class TemperatureAndHumidityBLEReceiveManager @Inject constructor(
    ...
    //todo блютуз данные датчиков
    private val DEVICE_NAME = "ЗДЕСЬ_БУДЕТ_НАЗВАНИЕ_БЛЮТУЗ_ДЕВАЙСА"
        ...
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            if(status == BluetoothGatt.GATT_SUCCESS){
                if(newState == BluetoothProfile.STATE_CONNECTED){
                    coroutineScope.launch {
                        data.emit(Resource.Loading(message = "Поиск..."))
                    }
                ...
                coroutineScope.launch {
                    data.emit(
                        Resource.Loading(
                            message = "Подключение к  $currentConnectionAttempt/$MAXIMUM_CONNECTION_ATTEMPTS"
                        )
                    )
                }
                if(currentConnectionAttempt<=MAXIMUM_CONNECTION_ATTEMPTS){
                    ...
                    coroutineScope.launch {
                        data.emit(Resource.Error(errorMessage = "Не удалось подключиться к устройству"))
                    }
                }
            }
        }
...
private fun enableNotification(characteristic: BluetoothGattCharacteristic){
    ...
    characteristic.getDescriptor(cccdUuid)?.let { cccdDescriptor ->
        if(gatt?.setCharacteristicNotification(characteristic, true) == false){
            Log.d("BLEReceiveManager","не удалось установить параметры уведомления")
            return
        }
...
override fun startReceiving() {
    coroutineScope.launch {
        data.emit(Resource.Loading(message = "Сканирование устройств..."))
    }
    isScanning = true
...
private fun disconnectCharacteristic(characteristic: BluetoothGattCharacteristic){
    ...
    characteristic.getDescriptor(cccdUuid)?.let { cccdDescriptor ->
        if(gatt?.setCharacteristicNotification(characteristic,false) == false){
            Log.d("TempHumidReceiveManager","не удалось установить параметры уведомления")
            return
        }
